...
Definition natCat_assoc {n m p q} (f : natCat n m) (g : natCat m p) (h : natCat p q) :
  comp natCat (comp natCat f g) h ≡ comp natCat f (comp natCat g h).
Proof. apply eq_Id, Nat.proof_irr. Qed.
...
Definition Pshmax@{a b} := Type@{psh|max(a,b)}.
...
Record dpdPsh@{a b} {A : Psh@{a}} := {
  ond :> forall n, on A n -> Type@{b} ;
  actd : forall {n m} (α : hom natCat n m) (a : on A m),
    ond m a -> ond n (act A α a)
}.
Arguments dpdPsh : clear implicits.
Symbol fromDpdPsh@{a b} : forall {A : Psh@{a}}, dpdPsh@{a b} A  -> A -> Psh@{b}.
Symbol toDpdPsh@{a b} : forall {A : Psh@{a}}, (A -> Psh@{b}) -> dpdPsh@{a b} A.
Rewrite Rule fromToDpdPsh := fromDpdPsh (toDpdPsh ?f) ==> ?f.
Rewrite Rule toFromDpdPsh := toDpdPsh (fromDpdPsh ?f) ==> ?f.

Record piPsh_on@{a b} {A : Psh@{a}} {B : dpdPsh@{a b} A} {n : natCat} :=
  {
    πmap :> forall {m} (α : hom natCat m n) (a : on A m), B m a ;
    πnatural : forall {m p} (α : hom natCat m n) (β : hom natCat p m) (a : on A m),
      actd B β a (πmap α a) ≡  πmap (comp natCat β α) (act A β a)
  }.
Arguments piPsh_on : clear implicits.


Definition piPsh_ext@{a b c| a <= c, b <= c} (A : Psh@{a}) (B : dpdPsh@{a b} A) : pshf_op_ext@{c}.
Proof.
  refine {| on_ext := piPsh_on A B ; act_ext n m α f := {| πmap p β a := f _ (comp natCat β α) a |} |}.
  intros; now rewrite natCat_assoc, f.(πnatural).
Defined.

Set Printing Universes.
Check (forall {A : Psh} {B : A -> Psh}, forall a : A, B a).

Axiom piPshEq@{a b c +} : forall {A : Psh@{a}} {B : A -> Psh@{b}}, from_ext@{c} (piPsh_ext@{a b c} A (toDpdPsh@{a b} B)) ≡ (forall (a : A), B a).

Symbol fromPiPsh@{a b c} : forall {A : Psh@{a}} {B : A -> Psh@{b}}, from_ext (piPsh_ext@{a b c} A (toDpdPsh B)) -> forall (a : A), B a.
Symbol toPiPsh@{a b c} : forall {A : Psh@{a}} {B : A -> Psh@{b}}, (forall (a : A), B a) -> from_ext (piPsh_ext@{a b c} A (toDpdPsh B)).
Rewrite Rule fromToPiPsh := fromPiPsh (toPiPsh ?f) ==> ?f.
Rewrite Rule toFromPiPsh := toPiPsh (fromPiPsh ?f) ==> ?f.



Record arrPshTy@{a b} {A : Psh@{a}} {B : Type@{b}} :=
  { arrPshTy_map :> forall n, on A n -> B ;
    arrPshTy_natural : forall {n m} (g : hom natCat n m) (a : on A m), 
      arrPshTy_map n (act A g a) ≡ arrPshTy_map m a
  }.
Arguments arrPshTy : clear implicits.
Symbol fromArrPshTy@{a b} : forall {A : Psh@{a}} {B : Type@{b}},
  arrPshTy A B -> A -> B.
Symbol toArrPshTy@{a b} : forall {A : Psh@{a}} {B : Type@{b}},
  (A -> B) -> arrPshTy A B.
Rewrite Rule fromToarrPsTy := fromArrPshTy (toArrPshTy ?f) ==> ?f.
Rewrite Rule toFromarrPsTy := toArrPshTy (fromArrPshTy ?f) ==> ?f.


Record piPsh@{a b c} {A : Psh@{a}} {B : arrPshTy@{a c} A Psh@{b}} {n : natCat} := {
  map : forall {m} (α : hom natCat m n) (a : on A m), on (B m a) m
}.

Symbol fromPiPsh@{a b} : forall {A : Psh@{a}} {B : A -> Psh@{b}},
  arrPshTy A B -> A -> B.
Symbol toPiPsh@{a b} : forall {A : Psh@{a}} {B : Type@{b}},
  (A -> B) -> arrPshTy A B.
Rewrite Rule fromToarrPsTy := fromArrPshTy (toArrPshTy ?f) ==> ?f.
Rewrite Rule toFromarrPsTy := toArrPshTy (fromArrPshTy ?f) ==> ?f.
...
Symbol arrPshTy@{a b} : forall (A : Psh@{a}) (B : Type@{b}), Pshmax@{a b}.
Symbol fromarrPshTy@{a b} : forall {A : Psh@{a}} {B : Type@{b}},
  arrPshTy A B -> A -> B.
Symbol toarrPshTy@{a b} : forall {A : Psh@{a}} {B : Type@{b}},
  (A -> B) -> arrPshTy A B.
Rewrite Rule fromToarrPsTy := fromarrPshTy (toarrPshTy ?f) ==> ?f.
Rewrite Rule toFromarrPsTy := toarrPshTy (fromarrPshTy ?f) ==> ?f.
Rewrite Rule on_arrPshTy : on (arrPshTy ?A ?B) ?n ==> 




Record PiPshTy@{a b} {A : Psh@{a}} {B : A -> Type@{b}} :=
  { piPshTy_map :> forall n (a : on A n), }


Symbol piPshTy@{a b} : forall (A : Psh@{a}) (B : A -> Type@{b}), Pshmax@{a b}.
Symbol fromPiPshTy@{a b} : forall {A : Psh@{a}} {B : A -> Type@{b}},
  piPshTy A B -> forall a : A, B a.
Symbol toPiPshTy@{a b} : forall {A : Psh@{a}} {B : A -> Type@{b}},
  (forall a : A, B a) -> piPshTy A B.
Rewrite Rule fromToPiPsTy := fromPiPshTy (toPiPshTy ?f) ==> ?f.
Rewrite Rule toFromPiPsTy := toPiPshTy (fromPiPshTy ?f) ==> ?f.
Rewrite Rule on_piPshTy : on (piPshTy ?A ?B) ?n ==> 
